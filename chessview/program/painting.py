...
def drawLine(image, line, color=(255, 255, 255), width=1):
    cv2.line(image, (line.x1, line.y1), (line.x2, line.y2), color, width)

def rescale(img, factor=0.2):
...
def debug_painting(frame, hor, vert, title='locateChessboard'):
    ...
    for line in hor + vert:
        drawLine(image, line, color=(120, 120, 120))

    cv2.imshow(title, image)
    cv2.waitKey(0)

def paintArrayPoints(array, color=[55,200,190], radius=5, image=None):    
    n, m, _ = array.shape
    if image is None:
    ...
    #cv2.imshow('paintArrayPoints Total on %s' % frame_number, image)
    #cv2.waitKey(0)
...
def display_hint(frame, vert, hor, hint, vert_m=None, hor_m=None):
    ...
    ideal_to_frame = makeInverseMatrix(hint)
    frame_to_ideal = hint
    ...
    for (row_index, line) in enumerate(hor):
        if hor_m is not None and row_index in hor_m:
            drawLine(image, hor[row_index], color=(255, 120, 120), width=2)
        else:
            drawLine(image, hor[row_index], color=(120, 120, 120))
    for (col_index, line) in enumerate(vert):
        if vert_m is not None and col_index in vert_m:
             drawLine(image, vert[col_index], color=(120, 255, 120), width=2)
        else:
            drawLine(image, vert[col_index], color=(120, 120, 120))
    ...
    cv2.imshow('Hinted on %s' % gvars.frame_number, image)
    cv2.waitKey(0)
    
    
    
    
def paintIdealBoard(matrix_to_frame, image=None, color=[150, 100, 100]):
    from program.grid import generateIdealBoard
    ideal_board = generateIdealBoard()
    real_board = cv2.perspectiveTransform(np.array(ideal_board), matrix_to_frame)
    paintArrayPoints(real_board, color=color, image=image)
